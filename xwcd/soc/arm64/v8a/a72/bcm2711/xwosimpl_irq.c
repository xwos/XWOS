...
{
        return armv8a_gic_irq_get_id(irqnbuf);
}

__xwbsp_code
xwer_t xwospl_irq_enable(xwirq_t irqn)
{
        return armv8a_gic_irq_enable(irqn);
}

__xwbsp_code
xwer_t xwospl_irq_disable(xwirq_t irqn)
{
        return armv8a_gic_irq_disable(irqn);
}

__xwbsp_code
xwer_t xwospl_irq_save(xwirq_t irqn, xwreg_t * flag)
{
        return armv8a_gic_irq_save(irqn, flag);
}

__xwbsp_code
xwer_t xwospl_irq_restore(xwirq_t irqn, xwreg_t flag)
{
        return armv8a_gic_irq_restore(irqn, flag);
}
