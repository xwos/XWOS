...
static
xwer_t stm32cube_usart3_drv_cfg(struct xwds_dmauartc * dmauartc,
                                const struct xwds_uart_cfg * cfg);
static
...
xwer_t stm32cube_usart3_drv_start(struct xwds_device * dev)
{
        ...
        MX_USART3_UART_Init();
        rc = MX_USART3_RXDMA_Start(dmauartc->rxq.mem, sizeof(dmauartc->rxq.mem));
        if (rc < 0) {
                goto err_rxdma_start;
        }
...
{
        xwds_dmauartc_drvcb_rxq_flush(dmauartc);
        MX_USART3_RXDMA_Start(dmauartc->rxq.mem, sizeof(dmauartc->rxq.mem));
}
...
void stm32cube_usart3_cb_rxdma_timer(struct xwds_dmauartc * dmauartc)
{
        ...
        tail = sizeof(dmauartc->rxq.mem) - MX_USART3_RXDMA_GetCounter();
        if (sizeof(dmauartc->rxq.mem) == tail) {
                xwds_dmauartc_drvcb_rxq_pub(dmauartc, 0);
        } else {
                xwds_dmauartc_drvcb_rxq_pub(dmauartc, tail);
