...
### 头文件保护宏
头文件保护宏格式为 `__文件路径_文件名_hxx__`，用下划线分隔：
...
### 文件头注释
每个文件必须以 Doxygen 风格的注释开头，包含文件描述、作者和版权信息，例如：
...
### 宏
- 头文件保护宏：`__xwos_文件路径_文件名_hxx__`（全部小写）
- 常量宏：全大写，下划线分隔
...
- **内存安全**：嵌入式环境需特别注意内存使用，应尽量避免使用 `new` 和 `delete` 以及 `malloc()` 和 `free()`
- **可移植性**：避免使用平台特定特性
- **性能考虑**：实时操作系统需保证确定性的执行时间
- **RAII 模式**：优先使用 RAII 模式管理资源
- **异常处理**：XWOS 不使用 C++ 异常，使用错误码返回值
- **避免使用异常和 RTTI**
- **使用显式构造函数避免隐式转换**
