 ...
 * @brief 应答错误对照表
 */
__xwmd_rodata const xwer_t xwssc_callback_rc[XWSSC_ACK_NUM] = {
        [XWSSC_ACK_OK] = XWOK,
 ...
 * @brief 数据长度编解码移位表
 */
__xwmd_rodata const xwu8_t xwssc_ecsdusz_shift_table[] = {
...
{
        xwssclogf(DEBUG,
                  "[RX] Publish a frame(%p): port:0x%X, frmsize:0x%u\n",
...
xwer_t xwssc_rx_frm_connect(struct xwssc * xwssc, union xwssc_slot * slot)
{
        ...
        proto[4] = sdupos[4];
        proto[5] = '\0';
        xwssclogf(DEBUG, "[RX] proto:%s-%d.%d.%d\n",
...
xwer_t xwssc_rx_ack_sdu(struct xwssc * xwssc, union xwssc_slot * slot)
{
        ...
        ack = sdupos[0];
        xwssclogf(DEBUG, "[RX] ID:0x%X, ACK:0x%X\n", rmtid, ack);
...
struct xwssc_carrier * xwssc_txq_carrier_alloc(struct xwssc * xwssc)
{
        ...
        if (caridx < 0) {
                ...
                XWSSC_BUG_ON(car->state != XWSSC_CRS_IDLE);
        }
        return car;
}
...
void xwssc_finish_tx(struct xwssc * xwssc, struct xwssc_carrier * car)
{
        ...
        if (rmtid != car->slot->tx.frm.head.id) {
                ...
                case XWSSC_ACK_EALREADY:
                        xwssclogf(DEBUG, "[TX] Msg is already TX-ed!\n");
                        xwaop_add(xwu32_t, &xwssc->txq.cnt, 1, NULL, NULL);
                        xwaop_write(xwu32_t, &car->state, XWSSC_CRS_FINISH, NULL);
                        xwos_splk_lock(&xwssc->txq.notiflk);
                        if (car->slot->tx.ntfcb) {
...
xwer_t xwssc_txfsm(struct xwssc * xwssc)
{
        ...
        if (XWSSC_HWIFST_CONNECT & hwifst) {
                ...
                xwssclogf(DEBUG,
                          "[TX] Choose carrier(%p), ID:0x%X, port:0x%X\n",
                          car,
