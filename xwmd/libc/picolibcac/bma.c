...
extern struct xwmm_bma * picolibcac_bma;
...
void * malloc(size_t size)
{
        ...
        rc = xwmm_bma_malloc(picolibcac_bma, size, &mem);
        errno = -rc;
        return mem;
}
...
void * realloc(void * mem, size_t size)
{
        ...
        rc = xwmm_bma_realloc(picolibcac_bma, size, &mem);
        errno = -rc;
        return mem;
}
...
void * calloc(size_t elem_nr, size_t elem_sz)
{
        ...
        rc = xwmm_bma_malloc(picolibcac_bma, total, &mem);
        errno = -rc;
        if (XWOK == rc) {
...
void * memalign(xwsz_t alignment, xwsz_t size)
{
        ...
        mem = NULL;
        rc = xwmm_bma_memalign(picolibcac_bma, alignment, size, &mem);
        errno = -rc;
        return mem;
}
...
void * valloc(size_t size)
{
        return memalign(LIBC_BMA_PAGE_SIZE, size);
}
...
void free(void * mem)
{
        ...
        rc = xwmm_bma_free(picolibcac_bma, mem);
        errno = -rc;
}
...
struct mallinfo mallinfo(void)
{
        ...
        mi.arena = picolibcac_bma->zone.size;
        return mi;
}
...
size_t malloc_usable_size(void * mem)
{
        ...
        rc = xwmm_bma_malloc_usable_size(picolibcac_bma, mem, &sz);
        if (rc < 0) {
                sz = 0U;
        }
        return sz;
