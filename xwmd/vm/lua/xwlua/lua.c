...
#include "xwmd/vm/lua/src/lprefix.h"
...
static void print_version (void) {
  lua_writestring(XWOS_COPYRIGHT, strlen(XWOS_COPYRIGHT));
  lua_writeline();
...
int pmain(lua_State * L)
{
  ...
  xwlua_openlibs(L);  /* open standard libraries */
...
xwer_t xwlua_task(void * arg)
{
  xwer_t rc;
  int status, result;
  struct xwlua_arg * luarg = arg;
  lua_State * L;

  L = luaL_newstate();
  if (L == NULL) {
    l_message(luarg->argv[0], "cannot create state: not enough memory");
    return -ENOMEM;
  }
  lua_gc(L, LUA_GCSTOP);  /* stop GC while building state */
  lua_pushcfunction(L, &pmain);  /* to call 'pmain' in protected mode */
  lua_pushinteger(L, luarg->argc);  /* 1st argument */
  lua_pushlightuserdata(L, luarg->argv); /* 2nd argument */
