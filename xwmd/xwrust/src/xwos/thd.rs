...
//!
//! XWOS RUST的线程分为 **动态线程** 与 **静态线程** 。无论哪种线程创建时都是 **连接态（joinable）** 的，
//! 如果未将线程的 **Handle** 绑定到变量上，线程将自动转变为 **分离态（detached）** 。
//!
//!
//! # 动态线程
//!
...
//! 其生命周期结束后的 [`drop()`] 方法会自动将动态线程转变为 **分离态(detached)** 。此线程运行结束后，其资源会被操作系统自动回收。
//!
//!
//! 动态线程也可以是 **可连接的(joinable)** ，可以将返回的 [`DThdHandle`] 绑定在变量上，防止 [`drop()`] 方法被调用。
...
//! [`DThdElement`] 由系统创建，并用作参数传递给闭包，用户可在闭包内直接使用。
//!
...
//! ## 静态线程的线程函数
//!
//! 定义RUST闭包时，实际上会生成一个隐藏的结构体，然后将捕获的环境放在结构体内。
//! 闭包其实是这个隐藏结构体的变量，并且是在函数栈上建立的，其生命周期只限于函数的局部。
//! 因此，不能将闭包直接转移到静态线程的线程函数内部，因为生命周期不够长。
//!
//! 如何延长闭包生命周期？RUST语言的常用技巧是借助于 [`Box<T>`] 或 [`Arc<T>`] 将其放入堆中。
//! 但若使用了 [`Box<T>`] 或 [`Arc<T>`] 就违背了“不使用动态内存分配器”的初衷。
//!
//! 综上，静态线程的线程函数只能是 **普通的函数** 或 **不捕获任何环境的闭包** 。
//! 只能访问 **静态生命周期的全局变量** 。
