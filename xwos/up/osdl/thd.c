                         ...
                         xwosdl_thd_f thdfunc, void * arg)
{
        ...
        XWOS_VALIDATE((thdfunc), "nullptr", -EFAULT);
                           ...
                           struct xwosdl_thd_attr * attr)
{
        ...
        XWOS_VALIDATE((attr), "nullptr", -EFAULT);

        rc = xwup_thd_acquire(thd, tik);
        if (XWOK == rc) {
                xwup_thd_get_attr(thd, (struct xwup_thd_attr *)attr);
                xwup_thd_put(thd); // cppcheck-suppress [misra-c2012-17.7]
        }
        return rc;
}
                                 ...
                                 struct xwosdl_thd_stack_info * info)
{
        ...
        rc = xwup_thd_acquire(thd, tik);
        if (XWOK == rc) {
                xwup_thd_get_stack_info(thd, (struct xwup_skdobj_stack *)info);
                xwup_thd_put(thd); // cppcheck-suppress [misra-c2012-17.7]
        }
        return rc;
}
...
xwer_t xwosdl_thd_get_stack_usage(struct xwosdl_thd * thd, xwsq_t tik, xwsz_t * usage)
{
        ...
        rc = xwup_thd_acquire(thd, tik);
        if (XWOK == rc) {
                xwup_thd_get_stack_usage(thd, usage);
                xwup_thd_put(thd); // cppcheck-suppress [misra-c2012-17.7]
        }
        return rc;
}
