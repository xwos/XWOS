...
class DSwt
{
  ...
  public:
     ...
     * @brief 动态软件定时器析构函数
     */
    virtual ~DSwt() { xwos_swt_delete(mSwtDesc); }
    /**
     * @brief 获取动态软件定时器构造的结果
     * @note
     * 动态对象的创建可能会因为内存不足构造失败。
     */
    xwer_t getDSwtCtorRc() { return mCtorRc; }
    /**
     * @brief 启动软件定时器
    ...
    xwer_t start(xwtm_t origin, xwtm_t period)
    {
        ...
        if (TCpu == Cpu::getLocalCpuId()) {
            rc = xwos_swt_start(mSwtDesc.swt, origin, period,
     ...
     * + 上下文：任意
     */
    xwer_t stop() { return xwos_swt_stop(mSwtDesc.swt); }
    /**
     * @brief 获取XWOS对象指针
     */
    struct xwos_swt * getXwosObj() { return mSwtDesc.swt; }
  ...
  private:
    static void sSwtAlarmFunction(struct xwos_swt * swt, DSwt * obj) { obj->swtAlarmFunction(); }
};
