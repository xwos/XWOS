...
xwer_t xwos_tt_remove_locked(struct xwos_tt * xwtt, struct xwos_ttn * ttn)
{
        ...
        if (__unlikely((NULL == ttn->cb) || (xwtt != ttn->xwtt))) {
                ...
                ttn->xwtt = NULL;
                xwaop_store(xwsq_t, &ttn->wkuprs,
                            xwmb_modr_release, XWOS_TTN_WKUPRS_INTR);
                ttn->cb = NULL;
                rc = OK;
        }
        return rc;
...
void xwos_tt_bh(struct xwos_tt * xwtt)
{
        ...
        xwlib_bclst_itr_prev_entry_del(ttn, &xwtt->timeout, struct xwos_ttn, rbb) {
                ...
                cb = ttn->cb;
                xwaop_store(xwsq_t, &ttn->wkuprs,
                            xwmb_modr_release, XWOS_TTN_WKUPRS_TIMEDOUT);
                ttn->cb = NULL;
                xwlk_sqlk_wr_unlock_cpuirq(&xwtt->lock);
...
{
        xwer_t rc;
        hwt->timetick = (xwtm_t)(-(XWSMPCFG_SYSHWT_PERIOD));
        hwt->irqrsc = NULL;
        hwt->irqs_num = 0;
...
{
        return (xwu64_t)xwos_syshwt_get_timetick(hwt) / XWSMPCFG_SYSHWT_PERIOD;
}
...
void xwos_syshwt_task(struct xwos_syshwt * hwt)
{
        ...
        hwt->timetick = xwtm_add(hwt->timetick, XWSMPCFG_SYSHWT_PERIOD);
